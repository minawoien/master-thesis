...
# Process plaintexts
def process_plaintext(ainput0, ainput_p, p_bits, public_bits):

    ainput = concatenate([ainput0, ainput_p], axis=1)
...
einput1a = Input(shape=(p1_bits, )) # plaintext 1a
einput1b = Input(shape=(p1_bits, )) # plaintext 1b
...
econv4 = Conv1D(filters=4, kernel_size=1, strides=1, 
                padding=pad, activation='relu')(econv3) 
eflattened = Flatten()(econv4)
efinal_dense = Dense(4, activation='sigmoid')(eflattened)  # 4 units for 4 plaintext options

eve = Model([einput0, einput1, einput1a, einput1b, einput2a, einput2b], efinal_dense, name='eve')
...
eveout = eve([HOout, ainput0, einput1a, einput1b, einput2a, einput2b])
